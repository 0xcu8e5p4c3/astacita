      ...
      <div class="flex-1">
        ...
        <div class="carousel carousel-end rounded-box">
            @foreach($trending as $item)
              <a href="{{ route('category.show', $item->slug) }}" class="carousel-item relative">
          ...
          @if(isset($articles) && count($articles) > 0)
              @foreach ($articles as $article)
                  <div class="relative bg-white shadow-md rounded-lg overflow-hidden flex flex-col md:flex-row mb-2 ">
                      <a href="{{ route('category.show', $article->slug) }}" class="w-full md:w-1/3">
                      ...
                      <div class="p-4 w-full md:w-2/3 flex flex-col justify-between">
                          <div>
                              ...
                              <h3 class="text-lg font-semibold mt-1">
                                  <a href="{{ route('category.show', $article->slug) }}" class="hover:text-blue-500">
                                      {{ $article->title }}
                                  </a>
                              </h3>
